...
//给定以弧度为单位的航向，将转向伺服输出从-1返回到+1
//将rate_max_rads设置为非零值，以限制所需的转弯率
//返回值通常在-1.0到+1.0的范围内，但可以更高或更低
...
{
    // calculate the desired turn rate (in radians) from the angle error (also in radians) 根据角度误差（也以弧度为单位）计算所需的转弯率
    float desired_rate = get_turn_rate_from_heading(heading_rad, rate_max_rads);

    return get_steering_out_rate(desired_rate, motor_limit_left, motor_limit_right, dt);
}

// return a desired turn-rate given a desired heading in radians 给定以弧度为单位的期望航向，返回期望的转弯率
float AR_AttitudeControl::get_turn_rate_from_heading(float heading_rad, float rate_max_rads) const
{
    ...
    // Calculate the desired turn rate (in radians) from the angle error (also in radians) 根据角度误差（也以弧度为单位）计算所需的转弯率（以弧度为准）
    float desired_rate = _steer_angle_p.get_p(yaw_error);

    // limit desired_rate if a custom pivot turn rate is selected, otherwise use ATC_STR_RAT_MAX 如果选择了自定义枢轴转弯率，则使用limit desired_rate，否则使用ATC_STR_RAT_MAX
